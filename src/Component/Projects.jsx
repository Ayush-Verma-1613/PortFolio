...
// Project Card Component - Premium Style - Mobile Optimized
const ProjectCard = ({ project, index, isVisible, isExpanded, onToggle }) => {
  const [isHovered, setIsHovered] = useState(false);

  return (
    <div
      className={`group relative bg-white/5 backdrop-blur-md border border-white/20 rounded-md sm:rounded-lg lg:rounded-xl overflow-hidden hover:border-cyan-400 transition-all duration-500 transform hover:scale-[1.02] ${
    ...
    >
      ...
      <div className="absolute top-1 right-1 sm:top-1.5 sm:right-1.5 z-20">
        <span className={`px-1 py-0.5 sm:px-1.5 sm:py-0.5 rounded-full text-[8px] sm:text-[9px] font-semibold ${
          project.status === 'Live' 
      ...
      {/* Card Content - Very compact for mobile */}
      <div className="relative z-10 p-1.5 sm:p-2 md:p-3">
        
        {/* Project Screenshot - Smaller on mobile */}
        <div className="relative w-full h-20 sm:h-24 md:h-32 rounded-md overflow-hidden bg-gray-800/50 backdrop-blur-sm mb-1.5 sm:mb-2">
          {project.image ? (
            <>
              <img
                src={project.image}
          ...
          <div className="absolute bottom-0.5 left-0.5 sm:bottom-1 sm:left-1">
            <span className="px-1 py-0.5 sm:px-1.5 sm:py-0.5 rounded-full text-[8px] sm:text-[9px] font-medium bg-black/70 backdrop-blur-sm text-cyan-300 border border-cyan-400/30">
              {project.category}
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between mb-1 sm:mb-1.5">
          <h3 className="text-xs sm:text-sm md:text-base font-serif font-bold text-white group-hover:text-cyan-300 transition-all duration-300">
            {project.title}
          </h3>
          <span className="text-[8px] sm:text-[9px] text-purple-300 font-serif">{project.year}</span>
        </div>

        {/* Description - Very compact */}
        <p className="text-gray-200 leading-relaxed mb-1.5 sm:mb-2 text-[9px] sm:text-[10px] md:text-xs font-serif line-clamp-2">
          {isExpanded ? project.longDescription : project.description}
        </p>

        {/* Highlights - Only show on expanded */}
        {isExpanded && (
          <div className="mb-1.5 sm:mb-2 animate-fade-in">
            <h4 className="text-[9px] sm:text-[10px] font-serif font-semibold text-cyan-300 mb-1">Key Features:</h4>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-0.5">
              {project.highlights.map((highlight, idx) => (
                <div key={idx} className="flex items-center gap-0.5 sm:gap-1 text-[8px] sm:text-[9px] text-gray-300">
                  <span className="text-cyan-300 text-[9px]">✓</span>
                  <span>{highlight}</span>
                </div>
              ))}
            </div>
        ...
        <div className="mb-1.5 sm:mb-2">
          <h4 className="text-[9px] sm:text-[10px] font-serif font-semibold text-gray-300 mb-0.5 sm:mb-1">Tech Stack:</h4>
          <div className="flex flex-wrap gap-0.5 sm:gap-1">
            {project.tech.map((tech, idx) => (
              <span
                key={idx}
                className="px-1 py-0.5 sm:px-1.5 sm:py-0.5 bg-white/10 backdrop-blur-sm text-cyan-300 rounded-full text-[8px] sm:text-[9px] font-serif font-medium hover:bg-white/20 hover:text-white transition-colors duration-200 border border-white/20"
              >
                {tech}
              </span>
            ))}
        ...
        <div className="flex flex-col xs:flex-row items-stretch gap-0.5 sm:gap-1">
          <a
            href={project.link}
            target="_blank"
            rel="noopener noreferrer"
            className="flex-1 bg-cyan-600 text-white px-2 py-1 sm:px-2.5 sm:py-1.5 rounded-md font-serif font-semibold text-center text-[9px] sm:text-[10px] hover:bg-cyan-700 transition-all duration-300 transform hover:scale-105"
          >
            View Live →
          </a>
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="px-2 py-1 sm:px-2.5 sm:py-1.5 border border-purple-400 text-purple-300 rounded-md font-serif font-semibold text-center text-[9px] sm:text-[10px] hover:bg-purple-500 hover:text-white transition-all duration-300"
          >
            Code
          </a>
          <button
            onClick={() => onToggle(project.id)}
            className="px-1.5 py-1 sm:px-2 sm:py-1.5 text-gray-300 hover:text-cyan-300 transition-colors duration-300 rounded-md border border-white/20 hover:border-white/40 text-[8px] sm:text-[9px]"
  ...
  return (
    <>
      <style jsx>{`
        ...
        /* Desktop Background */
        .projects-background-desktop {
          background-image: url(/featureProjects.webp);
        ...
        /* Mobile Background - Different Image with contain */
        @media (max-width: 1023px) {
          .projects-background-mobile {
            background-image: url(/MobileFeatureProject.png);
      ...
      <section 
        id="projects" 
        className="relative min-h-screen flex items-start justify-center overflow-hidden py-20 sm:py-24 lg:py-20 px-3 sm:px-4 md:px-6 lg:px-8"
        style={{
          backgroundColor: '#000000'
        }}
      >
        {/* Desktop Background */}
        <div className="hidden lg:block absolute inset-0 projects-background-desktop"></div>
        
        {/* Mobile Background */}
        <div className="lg:hidden absolute inset-0 projects-background-mobile"></div>
